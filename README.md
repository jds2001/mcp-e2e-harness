...
**API-boundary capture (preferred — wire-level ground truth).** For drivers whose outbound API traffic the harness can redirect (claude-code, via `ANTHROPIC_BASE_URL`), every invocation routes through a local transparent recording proxy that records the actual `tools` array sent to the model — `api-surface.jsonl` beside the trace, summarized in `meta.json` as `api_surface`. Nothing model-mediated sits in the verification path, and recording an outgoing request changes nothing in the consumer's context, so it runs during scored cells. Only tool *names* are recorded — never headers, message content, or schemas. Since the recording contract's 2026-09-02 amendment (context accounting, for measurements like uscde-mcp's E15) every request line also carries `body_bytes` (the serialized request body size, with `body_bytes_basis` `decompressed` when the body parsed — after undoing gzip/deflate — or `wire` when it did not, in which case `content_encoding` names what was on the wire), and the one allowlisted response scalar: a request the driver itself made to a count-tokens endpoint (`/v1/messages/count_tokens`, `/v1/responses/input_tokens`) gets `count_tokens` (the numeric `input_tokens` the endpoint returned, or `null` with `count_tokens_note` saying why it could not be read) on its own line; extraction decodes whatever response encoding the driver negotiated (gzip, deflate, brotli, zstd where a codec is available — the recorder never narrows the negotiation, since that would modify traffic; DR-3), and no other response is ever looked at. `meta.json`'s `api_surface` digest adds `requests_readable` (lines whose body parsed), `count_tokens_calls`, and `count_tokens_unread` (calls whose scalar came back null — a failed read is never an absent call). A disallowed builtin observed on the wire is an instrument breach (the row's claims are not tool-attributable); an invocation whose capture recorded zero model requests is *unverified*, never clean.
