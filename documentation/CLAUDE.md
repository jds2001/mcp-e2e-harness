...
- **The spec's acceptance record is the durable account; reports and run directories are disposable.** Write each record so it stands alone: the values read, the files they came from, what disagreed and how it was settled. A citation to a report or a run directory is provenance, never a substitute for stating what was observed. Maintainer's ruling 2026-09-23 (Q13), after a test reorganization moved the reports under gitignored `runs/`.
- **An acceptance run exercises every stage that consumes what changed.** A layout or artifact change is accepted only from a run that also declared at least one check and, where rows can carry them, one measurement — the reporting stages are consumers of the layout too. Earned 2026-09-22 (DR-4): WO-9 changed the row layout and all eleven acceptance manifests had no checks, so the checks evaluator first met the new layout in the maintainer's live run and killed it.
