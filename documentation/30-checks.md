...
**Report granularity — per cell, and a failure names its row (ruled 2026-09-21, spec session).** The four outcomes are reported per check **per cell**, each with its own matched count, beside the run-wide roll-up; and a failure reference is (cell, prompt id, repetition when the run has them — `10-harness.md` → repeats — and record index), never a bare index — `index` is a position within one row's trace, and a run holds many traces, so a bare index names nothing. **A failure reference is read from the row's record, never from its path (ruled 2026-09-22, DR-4).** `cell`, `prompt_id`, `repetition`, and `attempt` come from the row's `meta.json`; the row's path is recorded as `invocation` for locating the files and is parsed for nothing. A reference names `attempt` whenever the row has one, so a replaced slot's scored attempt is locatable without the run manifest. The run-wide roll-up is the worst outcome over cells in the order error, fail, pass, vacuous, with vacuous only when every cell is vacuous. Why this is a ruling and not a nicety: the text above said "per run, per check", and the observed report is exactly that — `runs/2026-09-18-loop-smoke/checks-report.json` covers three cells and carries one outcome per check — so one cell that fails a check hides whether every other cell passed it. The uscode-mcp spec session reached this from the other side (2026-09-21): one of its A/B arms fails a check on every record by design, and it asked whether a check can be scoped to cells. **It cannot, and that is deliberate:** the arm's server really does violate the contract the check asserts, so the fail is a true observation, and scoping the check away would delete it. What the suite needs is for that fail to be legible as the arm's and not to mask the control arm, which the per-cell breakdown gives; the suite pins the expected per-arm outcome in its preregistration, as that session already did. Until the implementation conforms (WO-6), run such arms as separate runs (`--cells`), where a run-wide report is a per-cell report. Unobserved, stated as such: no check has yet failed in any run under `runs/`, so the shape of a failure entry has never been read from an artifact; WO-6's acceptance produces the first.

**WO-6 part A accepted (2026-09-21, spec session, from `runs/2026-09-21-wo6/`, report `tests/WO6-acceptance.md` (at the commit cited; moved to `runs/acceptance/` by d4a6ebb, 2026-09-23) read second; commit 5d0364a on `implement/wo-6`; 324 tests pass under the spec session's own run, 291 before; ruff clean; live spend $0).** Read from the artifacts: the two cells of the acceptance manifest differ in `env` and nothing else, and the manifest hash is one value across all four experiments; `after/run/checks-report.json` reports the one check as pass with 1 matched for `control`, fail with 1 matched for `treatment`, and fail with 2 matched at the roll-up, under `checks[].cells.<cell>` with `outcome`, `matched`, `failures`; the first failure entry ever read from an artifact here names `cell`, `prompt_id`, `index`, the pointer in `details`, and `repetition` on the repeated run (`invocation` is the row path); the two rows' `cell_id` strings differ in exactly the `env:` digest component and share `knobs:`, `setup:`, `selection:`; each row's `meta.json` records `env` verbatim; the scored trace of each arm returned that arm's value although `server.transport.env` set the same key to `transport-default`, so the cell's value wins as ruled in `20-manifest.md`; every experiment's probe cache holds one entry. The instruction above to run arms as separate runs is retired from commit 5d0364a on.
