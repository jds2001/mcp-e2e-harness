...
| `setup` | no | ordered list of `{tool, args}` calls the harness makes directly against your server before the prompt, never via a model turn; what ran and what it returned is recorded in the row's `meta.json` |
| `env` | no | extra environment variables for **your server's process in this cell only**, added to `server.transport.env` (on a key both set, the cell's value wins — specified 2026-09-21 and not yet verified in the implementation, so until it is, give an arm's variable to every arm and leave it out of `server`), with the same `{"$secret": …}` handling; part of the cell's identity |
...
**Getting N invocations.** At harness 1950fbb the runner invokes each (cell, prompt) pair **once per run**, and there is nothing in the manifest that repeats a cell — nor will there be: the number of repetitions belongs to the run and not to the instrument, so it never touches your manifest hash. State N in your preregistration.

- **Today:** ten invocations of a cell means ten runs of the same manifest into ten run directories: `mcp-e2e run --manifest … --cells my-arm --run-dir runs/e17/my-arm-r01`, then `-r02`, and so on. The manifest hash is identical across them, which is what makes the rows comparable. Nothing aggregates across run directories for you: collect `answer_sha256_16` from each row's `meta.json`, and the number of distinct values is your distinct-answer count. The calibration-probe verdict is cached, so repeats do not re-pay it when they share a cache directory (`--loop-probe-cache`; the default sits beside the run directory).
- **Specified 2026-09-21 and ordered, not yet in the harness — check `mcp-e2e run --help` for it:** `--repeats N` invokes every selected pair N times in one run. Every repetition is a whole fresh invocation (new working directory, new server process, `setup` re-run). The runner makes pass 1 over all selected cells, then pass 2, so arms in one run are interleaved in time. With the flag, a row's directory gains a level, `<cell>/<group>/<prompt id>/r01/`; without it, nothing changes. Each row's `meta.json` carries `repetition`, and the run manifest's per-prompt `answers` then holds the real count: `invocations`, `answered` (non-empty answers), and `distinct` over the answered rows. The budget estimate multiplies by N. Rows from a `--repeats` run and rows from separate runs of the same manifest are comparable with each other.
...
- **A check cannot be scoped to cells, by design.** Checks select records by tool and by predicates on the record, never by cell. If one arm's server violates a check on every record, that fail is a true observation of that arm, and hiding it would delete it. Pin the expected per-arm outcome in your preregistration ("arm X fails check Y on every record: the arm's signature, not a finding") — the uscode-mcp suite does exactly this. One caution until the harness catches up: the checks report currently gives one outcome per check for the **whole run**, so a by-design fail in one arm hides whether the other arms passed. Per-cell outcomes are specified (2026-09-21) and ordered; until they land, run each arm as its own run (`--cells e17-control`), which you are doing anyway while repeats mean separate runs. Once both land, put the arms in one `--repeats` run: they are then interleaved in time, which is the better design.

### What a loop row records, and where

A row is one (cell, prompt) invocation; its directory is `<run-dir>/<cell>/<group>/<prompt id>/` (with a further `rNN/` level in a `--repeats` run, once that flag exists). Everything in "Runs" below applies to loop rows; these are the loop-specific readings.
...
| the distinct-answer count within one run | `run-manifest.json` | `loop_cells.<cell>.answers` (`invocations`, `distinct`, `digests`), keyed by prompt. A run without `--repeats` invokes each prompt once, so this reads 1 of 1; across run directories, aggregate `answer_sha256_16` yourself |
