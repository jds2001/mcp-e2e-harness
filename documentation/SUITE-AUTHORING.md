...
| a crowded row whose pre-turn never reached the mid-task state | `meta.json` | today (harness e2ae37b): `harness_failure` beginning `crowding pre-turn exited`, `exit_status: -1`, no answer; the reason is in the row's `loop-result-crowding.json` → `consumer_limit` and `crowding.json` → `stderr_tail`. The scored prompt was never sent, so there is nothing to score and nothing to count against your server: leave the row out, and count it when you report how many repetitions you obtained. Seen when the pre-turn itself ended with no content (the row above, one turn early). Leaving these rows out does not skew the rest — the pre-turn never sees your prompt |
