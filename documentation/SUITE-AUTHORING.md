...
| a turn that ended with no content at all | `meta.json` | **Known defect through harness 5d0364a:** when the model's final message has null content (seen when `openai/gpt-oss-120b` wrote its next tool call into its reasoning and stopped — 7 of 26 rows in one run), the harness writes the text `null` to `answer.txt` and counts it as an answer. Such a row has `answer_chars: 4` and `answer_sha256_16: 74234e98afe7498f`; treat it as a consumer that failed to answer, and exclude that digest from distinct-answer counts. The fix is specified and ordered: an empty `answer.txt`, no digest, not counted in `answered`, and a recorded outcome with cause `null_final_content`. The harness does not retry such a turn, and `loop-scaffold@1` never will — a rescue would change what the floor measures, so it could only arrive as a new scaffold version |
