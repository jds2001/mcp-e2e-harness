...
| why the model did what it did — asked afterwards, in the same context | `interview/NN/` under the row (from WO-13, when built): `questions.json`, `interview-session.json`, its own `api-surface.jsonl`, `trace.jsonl`, `meta.json` | `mcp-e2e interview --row <row dir> --ask "…"` resumes the row's session under the same scaffold, pin, and knobs with the tools mounted. What comes back is the model's self-report, never an observation: it enters your spec as a claim to preregister a measurement against (S10, S21), and nothing in the run aggregates it |
