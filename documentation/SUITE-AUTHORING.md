...
You are authoring a **suite**: the server-specific half of an end-to-end verification setup whose other half is the generic harness. The harness's repository is `model-e2e-harness`; the command it installs is `mcp-e2e`. This document is the contract, written so that you can author a manifest from it alone — if you had to open another harness file to write a cell, that is a defect in this document, and the harness spec session wants to hear about it. The harness spec (`documentation/` in the harness repo) is normative if the two ever disagree. First distilled 2026-08-31; last revised 2026-09-22, against harness commit b93c9b0. Three things below need that commit or later — `--repeats`, per-cell check outcomes, and `env` in `cell_id`; if `mcp-e2e run --help` shows no `--repeats`, your harness predates them.
...
| a row the endpoint refused (rate limit or outage) until the loop gave up | `meta.json` | A harness failure, never a consumer outcome: `harness_failure` set, `consumer_limit: null`, empty answer, the row counted under `failures` and not in `answered`. Through harness b93c9b0 the failure text begins `runner exited 2:` with the HTTP 429 inside a stderr tail and `loop.consumer_error`; the specified form leads with `upstream_unavailable` and a block naming the status, provider, and attempts. A 429 the loop recovered from mid-turn shows only as `error_responses` entries and `(absent)` finish reasons on those wire lines, and the row is ordinary. From WO-12 on, such a slot is re-run as a whole fresh invocation under the same attempt counter as an unmet pre-turn (`--precondition-retries`, default 3), with the refused attempts kept under `attempt-NN/`; through harness b93c9b0 it is not, so an arm can come up short — state the row count per arm when you compare |
