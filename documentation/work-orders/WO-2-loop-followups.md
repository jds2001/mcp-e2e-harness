...
WO-1 is accepted: the loop driver is verified for pinned cells and `loop-scaffold@1` is pinned in `40-instruments.md`. Every finding in the WO-1 report §5 received a ruling; this work order carries the rulings that need code. All are small. No live spend is required except item 5, capped at $0.05.
...
5. **Re-probe `mistralai/mistral-small-2603` @ `mistral`** with `mcp-e2e probe-loop` on a later day; record the verdict under `runs/`. If broken again, say so; do not strike it.
...
Artifacts, not the summary: one real or fake-upstream example line per new key (the `consumer_limit` row, a product row's family mark, a `duration_ms` line, the `pre_run` basis statement, the pin-identity fields), the test names covering each item, the mistral re-probe's `probe.json`, and total live spend.
