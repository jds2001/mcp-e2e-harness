...
**Accepted 2026-09-21** from `runs/2026-09-21-wo8/`, rows regenerated by the spec session; commit e2ae37b. The exemption did differ from the ruling (a product row was exempted) and now conforms. Record, including what the three rows do not isolate, in `10-harness.md` → Layer 2; two manifest observations that predate this work order are Q10 in `90-open-questions.md`. **Work-order weakness, the spec session's:** the mismatch row ordered here was already BROKEN under S7 before WO-8, so it does not isolate the new surface condition; the conditions that bind alone are covered by hermetic tests only.
