...
7. **The flag.** `--repeats N`, integer ≥ 1; anything else is a usage error before any invocation. Absent means one invocation per pair with today's layout, byte-for-byte: a run without the flag must produce the same paths and the same artifact keys as before, plus the two null fields in item 10.
